...
predicted_var_garch <- numeric(length(portfolio_returns_vec))
predicted_var_garch[1] <- omega / (1 - alpha - beta) # Long-run variance

for (i in 2:length(portfolio_returns_vec)) {
  predicted_var_garch[i] <- omega +
    alpha * portfolio_returns_vec[i - 1]^2 +
...
portfolio_returns %>%
  mutate(
    ...
    variance_pred_garch = predicted_var_garch
  ) %>%
  filter(date > ymd("2020-12-31")) %>%
  pivot_longer(starts_with("variance")) %>%
  ggplot(aes(x = date, y = value, color = name)) +
  geom_line() +
  scale_y_continuous(transform = scales::pseudo_log_trans(sigma = 0.0001))
```

Find good parameters for GARCH
...
f_garch_ssr <- function(params, returns_vec, variances_vec) {
  ...
  if(alpha + beta > 1) return(1000)
  
  predicted_var <- f_garch(omega, alpha, beta, returns_vec, variances_vec)
...
test_optim <- optim(
  ...
  returns_vec = portfolio_returns_vec,
  variances_vec = portfolio_variance_vec,
  # Optimizing with the bounds fails, probably because parameter combinations with alpha + beta > 1 are tried.
  # method = "L-BFGS-B",
  # lower = c(1e-9, 1e-6, 1e-6),
  # upper = c(1000, 0.1, 0.8)
)
test_optim$par
```

Now plot with optimal parameters.
...
portfolio_returns %>%
  mutate(
    ...
    variance_pred_exp_smoothing = f_pred_var(0.94),
    variance_pred_garch = predicted_var_garch,
    variance_pred_garch_opt = f_garch(test_optim$par[1], test_optim$par[2], test_optim$par[3])
  ) %>%
  filter(date > ymd("2020-12-31")) %>%
  pivot_longer(starts_with("variance")) %>%
  ggplot(aes(x = date, y = value, color = name)) +
