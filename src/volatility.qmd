...
We can just try a brute force approach many different parameter combinations and see which one fits best. The following code applies about 250,000 parameter combinations. The accuracy can easily be improved by trying more parameter combinations (by setting `length.out = 500` for example). But then, the computational load increases dramatically.
...
Lets look at an example using values for $\omega$, $\alpha$ and $\beta$
