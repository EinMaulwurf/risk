...
Using the GARCH(1,1) model fitted above, we can also simulate possible future return paths. This could then be used for Value-at-Risk calculations, for example.
...
test_sim <- ugarchsim(
  garch_fit, n.sim = garch_sim_horizon, m.sim = 500, 
  startMethod = "sample",
  prereturns = tail(portfolio_returns_vec, 1)
)

# as_tibble(test_sim@simulation$seriesSim) %>%
#   mutate(t = 1:n()) %>%
#   pivot_longer(cols = starts_with("V"), names_to = "sim_run", values_to = "return") %>%
#   mutate(return_cum = cumprod(1+return), .by = sim_run) %>%
#   ggplot(aes(x = t, y = return_cum, group = sim_run))+
#   geom_line(alpha = 0.1)
...
as_tibble(test_sim@simulation$seriesSim) %>%
  ...
  pivot_longer(cols = starts_with("V"), names_to = "sim_run", values_to = "return") %>%
  mutate(return_cum = cumprod(1+return), .by = sim_run) %>%
  slice_max(order_by = t, by = sim_run) %>%
  ggplot(aes(x = return_cum))+
  geom_density()
